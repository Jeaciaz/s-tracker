...
def create_spending(category_id: int, spending: schemas.SpendingCreate, db: Session = Depends(get_db)):
    return models.create_spending(db=db, spending=spending, category_id=category_id)
...
@router.put("/{spending_id}",
            ...
            status_code=status.HTTP_200_OK,
            response_model=schemas.Spending)
def update_spending(spending_id: int, spending: schemas.SpendingCreate, db: Session = Depends(get_db)):
...
@router.delete("/{spending_id}",
               ...
               status_code=status.HTTP_200_OK,
               response_model=None)
def delete_spenging(response: Response, spending_id: int, db: Session = Depends(get_db)):
    try:
