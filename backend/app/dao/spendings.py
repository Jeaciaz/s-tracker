...
class SpendingDAO(BaseDAO):
    def get_all(self, timestamp_from: int | None = None, timestamp_to: int | None = None, funnel_id: UUID4 | None = None) -> list[SpendingPublic]:
        ...
        query = (sa.select(spendings_table)
    ...
    def create(self, spending: SpendingCreate) -> UUID:
        result = self._connection.execute(sa.insert(spendings_table).values(**{**spending.dict(), 'id': str(uuid4())} | {'funnel_id': str(spending.funnel_id)}))
        return result.inserted_primary_key[0]


    def update(self, id: UUID, spending: SpendingCreate) -> None:
        result = self._connection.execute(sa.update(spendings_table).where(spendings_table.c.id == str(id)).values(**spending.dict()))
        if result.rowcount == 0:
            raise SpendingDoesNotExistException()


    def delete(self, id: UUID) -> None:
        result = self._connection.execute(sa.delete(spendings_table).where(spendings_table.c.id == str(id)))
        if result.rowcount == 0:
            raise SpendingDoesNotExistException()
